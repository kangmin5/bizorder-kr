...
export function PurchaseOrderPage() {
  ...
  const handleSaveDocument = async () => {
    const title = data.supplier.name 
      ? `${data.supplier.name} 발주서` 
      : `발주서 ${data.orderNumber}`;
    ...
    try {
      const docId = await saveDocument({
        ...
        type: 'purchase_order',
        title,
        clientName: data.supplier.name || '(미지정)',
        total: data.total,
      ...
      if (!currentDocumentId) {
        navigate(`/purchase-order?id=${docId}`, { replace: true });
      }
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col">
                  ...
                  <table className="w-full border-collapse border border-black mb-2 text-sm table-fixed" style={{ wordBreak: 'break-word' }}>
                    ...
                    <thead>
                      <tr className="bg-gray-100">
                        <th className="border border-black p-1.5 text-center relative">No<div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-400 print:hidden" onMouseDown={(e) => handleColumnResize('no', e.clientX, colWidths.no)} /></th>
                        <th className="border border-black p-1.5 text-center relative">품명<div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-400 print:hidden" onMouseDown={(e) => handleColumnResize('name', e.clientX, colWidths.name)} /></th>
                        <th className="border border-black p-1.5 text-center relative">규격<div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-400 print:hidden" onMouseDown={(e) => handleColumnResize('spec', e.clientX, colWidths.spec)} /></th>
                        <th className="border border-black p-1.5 text-center relative">단위<div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-400 print:hidden" onMouseDown={(e) => handleColumnResize('unit', e.clientX, colWidths.unit)} /></th>
                        <th className="border border-black p-1.5 text-center relative">수량<div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-400 print:hidden" onMouseDown={(e) => handleColumnResize('qty', e.clientX, colWidths.qty)} /></th>
                        <th className="border border-black p-1.5 text-center relative">단가<div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-400 print:hidden" onMouseDown={(e) => handleColumnResize('price', e.clientX, colWidths.price)} /></th>
                        <th className="border border-black p-1.5 text-center relative">공급가액<div className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-400 print:hidden" onMouseDown={(e) => handleColumnResize('amount', e.clientX, colWidths.amount)} /></th>
      ...
      <PDFPreviewModal
        ...
        pdfBlob={pdfBlob}
        fileName={`발주서_${data.orderNumber}.pdf`}
      />
    </div>
  );
}
