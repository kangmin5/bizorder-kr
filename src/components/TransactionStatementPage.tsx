...
const EditableInput = ({ 
  ...
  <input
    ...
    className={cn(
      "bg-transparent border border-transparent hover:border-gray-300 focus:border-blue-500 focus:outline-none px-1 py-0.5 w-full transition-colors",
      className
    )}
  />
);
...
export function TransactionStatementPage() {
  ...
  const renderThemeStyles = () => {
    ...
    return `bg-white border border-gray-200 ${fontClass}`;
  };
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col">
      <div className="border-b bg-white p-4 flex justify-between items-center z-10">
        <div className="flex items-center gap-4">
          ...
          {pages.length > 1 && (
            <div className="flex items-center gap-1 bg-green-50 rounded-lg px-2 py-1 text-sm">
              ...
              {pages.map((_, idx) => (
                <Button key={idx} variant={idx === 0 ? "default" : "outline"} size="sm" className="h-6 w-6 p-0 text-xs"
                  onClick={() => document.getElementById(`ts-page-${idx}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>
                  {idx + 1}
                </Button>
              ))}
        ...
        <ResizablePanel ref={sidebarRef} defaultSize={25} minSize={20} maxSize={40} collapsible={true}
          ...
          <div className="h-full overflow-auto">
            <div className="p-4 space-y-6">
              ...
              <Card>
                <CardHeader className="pb-3"><CardTitle className="text-base">용지 설정</CardTitle></CardHeader>
              ...
              {pages.map((page, pageIndex) => (
                <div key={pageIndex} id={`ts-page-${pageIndex}`}
                  ...
                  <div className="relative overflow-y-hidden">
                    <table className="w-full border-collapse border border-black mb-2 text-sm">
                      ...
                      <tbody>
                        {page.items.map((row) => {
                          ...
                          if (row.type === 'total') {
                            return (
                              <tr key="total" className="bg-green-100 font-bold">
                                <td colSpan={6} className="border border-black p-2 text-center">합 계</td>
                                <td className="border border-black p-2 text-right text-green-600">{data.total.toLocaleString()}</td>
                                <td className="border border-black p-2"></td>
                                <td className="border-0 print:hidden" data-html2canvas-ignore></td>
                              </tr>
                            );
                          ...
                          return (
                            <tr key={item.id} className="hover:bg-gray-50">
                              <td className="border border-black p-2 text-center">{itemIndex + 1}</td>
                              <td className="border border-black p-1">
                                <EditableInput value={item.name} onChange={(v) => handleItemChange(item.id, 'name', v)} placeholder="품명" />
                              </td>
                              <td className="border border-black p-1">
                                <EditableInput value={item.spec} onChange={(v) => handleItemChange(item.id, 'spec', v)} placeholder="규격" />
                              </td>
                              <td className="border border-black p-1">
                                <EditableInput value={item.unit} onChange={(v) => handleItemChange(item.id, 'unit', v)} />
                              </td>
                              <td className="border border-black p-1">
                                <EditableInput type="number" value={item.quantity} onChange={(v) => handleItemChange(item.id, 'quantity', Number(v))} className="text-right" />
                              </td>
                              <td className="border border-black p-1">
                                <EditableInput type="number" value={item.unitPrice} onChange={(v) => handleItemChange(item.id, 'unitPrice', Number(v))} className="text-right" />
                              </td>
                              <td className="border border-black p-2 text-right">{item.amount.toLocaleString()}</td>
                              <td className="border border-black p-1">
                                <EditableInput value={item.note} onChange={(v) => handleItemChange(item.id, 'note', v)} />
                              </td>
                              <td className="border-0 p-1 print:hidden" data-html2canvas-ignore>
                    ...
                    {page.showButton && (
                      <Button variant="outline" size="sm" onClick={addItem} className="print:hidden mb-4 w-full border-dashed border-2 hover:bg-gray-50 text-gray-600" data-html2canvas-ignore>
                  ...
                  {page.isLast && (
                    <div className="text-sm space-y-4 pt-2 flex-shrink-0 mt-2">
                      <div className="flex gap-4 items-start pt-2">
                        ...
                        <EditableTextarea value={data.remarks} onChange={(v) => setData({...data, remarks: v})} rows={3} placeholder="특이사항이나 비고를 입력하세요" className="flex-1 border border-gray-200 rounded p-2" />
                      </div>
                    </div>
                  )}
